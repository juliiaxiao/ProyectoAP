...
def create_fashion_cnn_low_dropout(input_shape=(28, 28, 3), num_classes=10):
    inputs = layers.Input(shape=input_shape)

    # Bloque 1 - Bajamos Dropout de 0.2 a 0.1
    ...
    x = layers.Dropout(0.1)(x) # <--- Dropout reducido

    # Bloque 2 - Eliminamos el SpatialDropout o lo dejamos al mínimo (0.05)
    x = layers.Conv2D(128, (3,3), padding='same', activation='relu')(x)
    x = layers.BatchNormalization()(x)
    x = layers.SpatialDropout2D(0.05)(x) # <--- Casi nulo para no perder canales
    x = layers.Conv2D(128, (3,3), padding='same', activation='relu')(x)
    x = layers.BatchNormalization()(x)
    x = cbam_block(x)
    x = layers.MaxPooling2D((2,2))(x)

    # Bloque 3 - Sin Dropout para máxima extracción
    x = layers.Conv2D(256, (3,3), padding='same', activation='relu')(x)
    x = layers.BatchNormalization()(x)
    x = cbam_block(x)
    x = layers.MaxPooling2D((2,2))(x)

    # Clasificador - Bajamos de 0.5 a 0.3
    x = layers.GlobalAveragePooling2D()(x)
    x = layers.Dense(512, activation='relu', kernel_regularizer=regularizers.l2(1e-4))(x)
    x = layers.BatchNormalization()(x)
    x = layers.Dropout(0.3)(x) # <--- Antes era 0.5
